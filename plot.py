...
class BigPlot(Plot):
    def create_plot(self, df, title, plot_properties):
        ...
        if plot_properties.show_extra_pen:
            p11 = self.ax1.plot(df['dtime'], df['max_charging_voltage'], '--', color='red',
                                label='Max charging volt. 1')
            p13 = self.ax2.plot(df['dtime'], df['ideal_temp'], '--', color='green', label='25C')
            p14 = self.ax3.plot(df['dtime'], df['capacity'], color='blue', label='Capacity 1')
        p16 = self.ax4.plot(df['dtime'], df[current_field], color='magenta', label='Current')
        p12 = self.ax2.plot(df['dtime'], df[temp_field], color='green', label='Temperature 1')
        p15 = self.ax1.plot(df['dtime'], df[voltage_field], color='red', label='Voltage 1')
        ...
        y_min = round(
            min([16] + df['temperature'].dropna().values.tolist())) - 1
        y_max = round(
            max([34] + df['temperature'].dropna().values.tolist())) + 1
        self.ax2.set_ylim(y_min, y_max)

        y_min = np.floor(np.concatenate([[11], df['voltage'].values]).max())
        y_max = np.ceil(np.concatenate([[14], df['voltage'].values, ]).max())
